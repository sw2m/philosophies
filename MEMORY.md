...
- **`test.yml`** — fires on `push` to `main` and on `pull_request`. Markdown lint, external link check (lychee), a structural sanity check that all nine Roman-numeral sections of `MEMORY.md` are present, and an `issue-linkage` job that fails the build if a PR body lacks a closing keyword + issue number (`Closes #N` / `Fixes #N` / `Resolves #N`) — every PR must declare which Phase 1c-vetted spec it satisfies. **Gating** — required by the branch-protection rule on `main`.
