...
This repo's `.github/workflows/` enforces VSDD on itself. Six workflows, mirroring the pipeline. For bot-trust predicates per event type, see [`.github/CLAUDE.md`](.github/CLAUDE.md). The path is named for Claude Code's auto-discovery contract (per #152): docs targeting LLM agents go at the agent-discovered path so they auto-load on every session, not at a topic-named path that requires explicit `Read`. A Gemini-discoverable mirror is tracked by #152.
