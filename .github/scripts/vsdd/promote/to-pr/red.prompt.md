...
the test files, output a JSON block summarizing what you did. The
JSON must be the LAST thing in your response, on its own line:

```json
{
  "files": ["path/to/new_test_file_1.ext", "path/to/new_test_file_2.ext"],
...
- One technical problem per tech-spec (per §VII). If the issue
  covers more than one, STOP and emit an empty `files: []` with
  empty commands; the pipeline will bail.
